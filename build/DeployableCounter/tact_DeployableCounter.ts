...
function dictValueParserAdd(): DictionaryValue<Add> {
 ...
 type DeployableCounter_init_args = {
    $$type: 'DeployableCounter_init_args';
    id: bigint;
}

function initDeployableCounter_init_args(src: DeployableCounter_init_args) {
    return (builder: Builder) => {
        let b_0 = builder;
...
const DeployableCounter_types: ABIType[] = [
...
const DeployableCounter_getters: ABIGetter[] = [
...
const DeployableCounter_receivers: ABIReceiver[] = [
    {"receiver":"internal","message":{"kind":"typed","type":"Add"}},
    {"receiver":"internal","message":{"kind":"typed","type":"Deploy"}},
]

export class DeployableCounter implements Contract {
    
    static async init(id: bigint) {
        return await DeployableCounter_init(id);
    }
    
    static async fromInit(id: bigint) {
        const init = await DeployableCounter_init(id);
        const address = contractAddress(0, init);
        return new DeployableCounter(address, init);
    }
    
    static fromAddress(address: Address) {
        return new DeployableCounter(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  DeployableCounter_types,
        getters: DeployableCounter_getters,
        receivers: DeployableCounter_receivers,
        errors: DeployableCounter_errors,
    };
