...
function dictValueParserAdd(): DictionaryValue<Add> {
 ...
 type StoppableCounter_init_args = {
    $$type: 'StoppableCounter_init_args';
    id: bigint;
}

function initStoppableCounter_init_args(src: StoppableCounter_init_args) {
    return (builder: Builder) => {
        let b_0 = builder;
...
const StoppableCounter_types: ABIType[] = [
...
const StoppableCounter_getters: ABIGetter[] = [
...
const StoppableCounter_receivers: ABIReceiver[] = [
    {"receiver":"internal","message":{"kind":"typed","type":"Add"}},
    {"receiver":"internal","message":{"kind":"typed","type":"Deploy"}},
]

export class StoppableCounter implements Contract {
    
    static async init(id: bigint) {
        return await StoppableCounter_init(id);
    }
    
    static async fromInit(id: bigint) {
        const init = await StoppableCounter_init(id);
        const address = contractAddress(0, init);
        return new StoppableCounter(address, init);
    }
    
    static fromAddress(address: Address) {
        return new StoppableCounter(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  StoppableCounter_types,
        getters: StoppableCounter_getters,
        receivers: StoppableCounter_receivers,
        errors: StoppableCounter_errors,
    };
